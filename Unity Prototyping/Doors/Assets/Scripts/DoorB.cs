...
public class DoorB : MonoBehaviour
{
    ...
    void Update ()
    {
        if (Input.GetKeyUp(KeyCode.Alpha2)) //sets keyHeld flag to false when key is released
            keyHeld = false;

        if (Input.GetKey(KeyCode.Alpha2) && doorClosed == true && keyHeld == false) //If the door is fully closed and has not just finished closing, this opens the door by scaling on the x axis towards the game object center
        {

            transform.localScale = new Vector3(transform.localScale.x - transform.localScale.x * (speed * Time.deltaTime), transform.localScale.y - transform.localScale.y * (speed * Time.deltaTime), transform.localScale.z); //negatively modifies only the x axis over time 
        ...
        if (Input.GetKey(KeyCode.Alpha2) && doorClosed == false && keyHeld == false) //if the door is not closed, closes the door.
        {
            doorOpen = false;
            transform.localScale = new Vector3(transform.localScale.x + transform.localScale.x * (speed * Time.deltaTime), transform.localScale.y + transform.localScale.y * (speed * Time.deltaTime), transform.localScale.z); //positively modifies only the x axis over time 
        ...
        {
            transform.localScale = new Vector3(transform.localScale.x + transform.localScale.x * (springy * Time.deltaTime), transform.localScale.y + transform.localScale.y * (speed * Time.deltaTime), transform.localScale.z); //more of the same shit as above, but faster
