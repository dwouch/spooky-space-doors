...
// BASED on http://wiki.unity3d.com/index.php/ExpressionParser
    ...
    public class ExpressionParser
    {
        ...
        IValue Parse(string aExpression)
        {
            ...
            if (index2a >= 0 && index2b >= 2)
            {
                ...
                int bracketIndex;
                if (int.TryParse(inner, NumberStyles.Any, CultureInfo.InvariantCulture, out bracketIndex) && bracketIndex >= 0 && bracketIndex < m_BracketHeap.Count)
                {
        ...
        private bool ValidIdentifier(string aExpression)
        {
            ...
            if (aExpression.Contains(" "))
                return false;
            if (!"_abcdefghijklmnopqrstuvwxyz§$".Contains(char.ToLower(aExpression[0])))
                return false;
