...
namespace AX
{
	...
	public class AXPolygon {
		...
		public static Mesh triangulate(Path path, AXTexCoords tex, int seglenBigInt = 100000,bool useGrid = false)
		{
			...
			if (seglenBigInt > 0)
					_points = AXGeometryTools.Utilities.path2polygonPts(Pather.segmentPath(Clipper.CleanPolygon(path), seglenBigInt));
				else 
...
//				return returnMesh;
		...
		public static Mesh triangulate(PolyNode polynode, AXTexCoords tex, int seglen = 100000, bool useGrid = false)
		{
			return triangulate(polynode.Childs, tex, seglen, useGrid);
		}
		...
		public static Mesh triangulate(List<PolyNode> childs, AXTexCoords tex, int seglenBigInt = 1000000, bool useGrid = false)
		{
...
//
        ...
        public static void addSteinerPointsAsGrid(ref Polygon _polygon, Paths paths, int seglen)
        {
            ...
            //long gridSizeLong = (long)(gridSize * AXGeometryTools.Utilities.IntPointPrecision);
            ...
            while (x < bounds.right)
            {
                while (y < bounds.bottom)
                {
                    ...
                    {
                        _polygon.AddSteinerPoint(new TriangulationPoint((double)x / (double)AXGeometryTools.Utilities.IntPointPrecision, (double)y / (double)AXGeometryTools.Utilities.IntPointPrecision));
                    }
